...
`)
	...
	if !strings.Contains(html, `<a href="https://www.luogu.com.cn/problem/P3403">P3403</a>`) {
		t.Fatalf("expected reference link to render, got: %s", html)
	}
	if strings.Contains(html, "[3]:") {
		t.Fatalf("expected reference definition line to be stripped, got: %s", html)
	}
...
```
`)
	post, err = core.LoadPost(filepath.Join(postsDir, "b.md"), "fallback")
	if err != nil {
		t.Fatalf("LoadPost error: %v", err)
	}
	html = string(post.HTML)
	if !strings.Contains(html, `[3]: https://example.com`) {
		t.Fatalf("expected reference definition inside code block to remain, got: %s", html)
	}
}

func TestMarkdownPreserveSpaces(t *testing.T) {
	tmp := t.TempDir()
	postsDir := filepath.Join(tmp, "posts")
	mustWriteFile(t, filepath.Join(postsDir, "a.md"), `---
title: "Spaces"
...
`)
	...
	if !strings.Contains(html, "a&nbsp; b") {
		t.Fatalf("expected double spaces to render, got: %s", html)
	}
...
`)
	...
	html = string(post.HTML)
	if !strings.Contains(html, "<p>&nbsp;&nbsp;ind</p>") {
		t.Fatalf("expected leading spaces to render, got: %s", html)
	}
}
